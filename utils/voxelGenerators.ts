...
function generateBox(map: Map<string, VoxelData>, xMin: number, xMax: number, yMin: number, yMax: number, zMin: number, zMax: number, col: number) {
    for (let x = xMin; x <= xMax; x++) {
        for (let y = yMin; y <= yMax; y++) {
            for (let z = zMin; z <= zMax; z++) {
                setBlock(map, x, y, z, col);
...
export const Generators = {
    ...
    ModernSofa: (): VoxelData[] => {
        const map = new Map<string, VoxelData>();
        const SY = CONFIG.FLOOR_Y + 1;
        const SOFA_COLOR = 0x88929b; // Slate-like modern color
        const LEG_COLOR = 0x222222;

        // Base/Seat
        generateBox(map, -10, 10, SY + 1, SY + 3, -4, 4, SOFA_COLOR);
        // Backrest
        generateBox(map, -10, 10, SY + 4, SY + 8, -4, -2, SOFA_COLOR);
        // Armrests
        generateBox(map, -11, -10, SY + 1, SY + 6, -4, 4, SOFA_COLOR);
        generateBox(map, 10, 11, SY + 1, SY + 6, -4, 4, SOFA_COLOR);
        
        // Legs
        generateBox(map, -10, -9, SY, SY, -3, -2, LEG_COLOR);
        generateBox(map, -10, -9, SY, SY, 2, 3, LEG_COLOR);
        generateBox(map, 9, 10, SY, SY, -3, -2, LEG_COLOR);
        generateBox(map, 9, 10, SY, SY, 2, 3, LEG_COLOR);

        // Cushions (subtle color diff)
        const CUSHION_COLOR = 0x9ba4ad;
        generateBox(map, -9, -1, SY + 4, SY + 4, -1, 3, CUSHION_COLOR);
        generateBox(map, 1, 9, SY + 4, SY + 4, -1, 3, CUSHION_COLOR);

        return Array.from(map.values());
    },

    ModernLamp: (): VoxelData[] => {
        const map = new Map<string, VoxelData>();
        const LY = CONFIG.FLOOR_Y + 1;
        const BASE_COLOR = 0x111111;
        const POLE_COLOR = 0x333333;
        const SHADE_COLOR = 0xf5f5dc; // Beige/White
        const LIGHT_COLOR = 0xffeebb;

        // Base
        generateSphere(map, 0, LY, 0, 3, BASE_COLOR, 0.3);
        
        // Pole
        generateBox(map, 0, 0, LY + 1, LY + 15, 0, 0, POLE_COLOR);
        
        // Arch
        generateBox(map, 0, 2, LY + 16, LY + 16, 0, 0, POLE_COLOR);
        generateBox(map, 3, 4, LY + 15, LY + 15, 0, 0, POLE_COLOR);
        
        // Shade
        generateSphere(map, 4, LY + 14, 0, 2, SHADE_COLOR, 0.5);
        generateSphere(map, 4, LY + 13, 0, 3, SHADE_COLOR, 0.5);
        
        // Light bulb
        setBlock(map, 4, LY + 12, 0, LIGHT_COLOR);
    ...
    Table: (): VoxelData[] => {
        ...
        const TOP_COLOR = 0xdddddd; // Light modern top
        const LEG_COLOR = 0x111111; // Dark metal legs

        // Table Top
        generateBox(map, -8, 8, TY + 6, TY + 6, -5, 5, TOP_COLOR);
        
        // Legs
        generateBox(map, -7, -6, TY, TY + 5, -4, -3, LEG_COLOR);
        generateBox(map, -7, -6, TY, TY + 5, 3, 4, LEG_COLOR);
        generateBox(map, 6, 7, TY, TY + 5, -4, -3, LEG_COLOR);
        generateBox(map, 6, 7, TY, TY + 5, 3, 4, LEG_COLOR);

        // Center piece / decoration
        setBlock(map, 0, TY + 7, 0, 0x55aa55); // Small plant
        setBlock(map, 0, TY + 8, 0, 0x449944);
